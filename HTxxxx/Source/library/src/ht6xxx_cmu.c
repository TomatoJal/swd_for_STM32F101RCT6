...
#elif defined HT6x2x  ||  defined  HT6x3x
             if((HT_CMU->SYSCLKDIV & CMU_SYSCLKDIV_SEL40M) != FALSE)
             {
                 sysclk =  32768*672*2;  
             }
             else
...
uint32_t HT_CMU_CPUClkGet(void)
{
    ...
    uint32_t cpuclk = 0, clksel = 0, cpuclkdiv = 0;       
...
*                                               f) CMU_CLKOUTSEL_PLL    PLL -->Clkout (for HT6x2x verH, HT6x3x)
...
FlagStatus HT_CMU_StopFlagGet(uint8_t STAFlag)
{
    ...
    if ((HT_CMU->CLKSTA & STAFlag) != FALSE)
    {       
...
void	HT_CMU_Prefetch_Set(FunctionalState NewState)
{
...
#elif  defined  HT6x3x//暂时这样改，等钜泉方面提供新库后调整【陈雷 2017.8.2】
		uint32_t writeProtect = RegisterWriteProtect[HT_CMU->WPREG & 0x01];    /*!< 保存当前写保护状态     */
    ...
    if (NewState != DISABLE)
    {       
        HT_CMU->PREFETCH = CMU_PREFETCH_CACHE_EN|CMU_PREFETCH_CTRL_2T|CMU_PREFETCH_FLASH_WR_SYN;/*!< 使能prefetch         */ 
    }
    else
    {
        HT_CMU->PREFETCH = 0;                          /*!< 关闭prefetch    */ 
    }  
